...
def normalize_phone(raw: str) -> str:
    """Приводит телефон к виду +7XXXXXXXXXX (если похоже на РФ), иначе — цифры."""
    raw = clean_text(raw)
...
def starts_with_lower(s: str) -> str:
    """Делаем так, чтобы строка начиналась с маленькой буквы (только первый символ)."""
    s = (s or "").strip()
    if not s:
        return ""
...
def extract_custom_fields(custom_fields_values):
    """Достаём phone/email по field_code, UTM/source — по названию поля."""
    out = {
        "phone": "",
        "email": "",
        "source": "",
...
def parse_name_fields(name: str):
    """Из name вытаскиваем channel, phone_from_name, name_clean."""
